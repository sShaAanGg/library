...
function getOrderField(_vm, column) {
  ...
  if (sortBy) {
    return _ctor.default.isArray(sortBy) ? sortBy[0] : sortBy;
  }

  return function (row) {
    var cellValue = _vm.getCellLabel(row, column);
    ...
    if (!sortType || sortType === 'auto') {
      ...
      return _ctor.default.toString(cellValue);
    }
...
var Methods = {
  ...
  getParentHeight: function getParentHeight() {
    var $el = this.$el,
        $xegrid = this.$xegrid;
    return Math.floor($xegrid ? $xegrid.getParentHeight() : _ctor.default.toNumber(getComputedStyle($el.parentNode).height));
  },
  ...
  loadChildren: function loadChildren(row, childRecords) {
    ...
    return this.createData(childRecords).then(function (rows) {
      ...
      _ctor.default.eachTree(rows, function (childRow, index, items, path, parent) {
        var rowid = getRowid(_this8, childRow);
        var rest = {
          row: childRow,
          rowid: rowid,
          index: -1,
  ...
  cacheColumnMap: function cacheColumnMap() {
    ...
    var handleFunc = function handleFunc(column, index, items, path, parent) {
      ...
      if (process.env.VUE_APP_VXE_TABLE_ENV === 'development') {
        if (_this9.showOverflow && column.showOverflow === false) {
          _tools.UtilTools.warn('vxe.error.errConflicts', ["table.show-overflow=".concat(_this9.showOverflow), "column.show-overflow=".concat(column.showOverflow)]);
        }

        if (_this9.showHeaderOverflow && column.showHeaderOverflow === false) {
          _tools.UtilTools.warn('vxe.error.errConflicts', ["table.show-header-overflow=".concat(_this9.showHeaderOverflow), "column.show-header-overflow=".concat(column.showHeaderOverflow)]);
        }

        if (_this9.showFooterOverflow && column.showFooterOverflow === false) {
          _tools.UtilTools.warn('vxe.error.errConflicts', ["table.show-footer-overflow=".concat(_this9.showFooterOverflow), "column.show-footer-overflow=".concat(column.showFooterOverflow)]);
        }
      }
  ...
  isUpdateByRow: function isUpdateByRow(row, field) {
    ...
    if (keepSource) {
      ...
      if (treeConfig) {
        ...
        var matchObj = _ctor.default.findTree(tableSourceData, function (item) {
          return rowid === getRowid(_this13, item);
        }, treeOpts);
        ...
        if (sortMultiple) {
          tableData = _ctor.default.orderBy(tableData, orderColumns.map(function (_ref3) {
            var column = _ref3.column,
                order = _ref3.order;
            return [getOrderField(_this14, column), order];
          }));
        } else {
    ...
    if (this.isActivated) {
      this.preventEvent(evnt, 'event.keydown', null, function () {
        ...
        if (operCtxMenu) {
          ...
          if (selected.column && selected.row && (0, _tools.isEnableConf)(selected.column.editRender)) {
  ...
  setCurrentRow: function setCurrentRow(row) {
    ...
    if (this.highlightCurrentRow) {
      _ctor.default.arrayEach(this.$el.querySelectorAll("[rowid=\"".concat(getRowid(this, row), "\"]")), function (elem) {
  ...
  setHoverRow: function setHoverRow(row) {
    ...
    _ctor.default.arrayEach(this.$el.querySelectorAll("[rowid=\"".concat(rowid, "\"]")), function (elem) {
  ...
  sort: function sort(sortConfs, sortOrder) {
    ...
    if (sortConfs) {
      if (_ctor.default.isString(sortConfs)) {
        sortConfs = [{
          field: sortConfs,
          order: sortOrder
        }];
    ...
    if (!_ctor.default.isArray(sortConfs)) {
      sortConfs = [sortConfs];
    }
    ...
    if (sortConfs.length) {
      ...
      (multiple ? sortConfs : [sortConfs[0]]).forEach(function (confs) {
        ...
        if (_ctor.default.isString(field)) {
  ...
  handleAsyncRowExpand: function handleAsyncRowExpand(row) {
    ...
    return new Promise(function (resolve) {
      ...
      _this33.expandOpts.loadMethod({
        ...
        _ctor.default.remove(_this33.expandLazyLoadeds, function (item) {
          return item === row;
        });
  ...
  scrollTo: function scrollTo(scrollLeft, scrollTop) {
    ...
    if (footerTargetElem && _ctor.default.isNumber(scrollLeft)) {
      footerTargetElem.scrollLeft = scrollLeft;
    }

    if (bodyTargetElem && _ctor.default.isNumber(scrollTop)) {
