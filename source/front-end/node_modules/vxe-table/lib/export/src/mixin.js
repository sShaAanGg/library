...
function getLabelData($xetable, opts, columns, datas) {
  ...
  if (treeConfig) {
    ...
    _ctor.default.eachTree(datas, function (item, rowIndex, items, path, parent, nodes) {
      ...
      if (isAllExpand || !parentRow || $xetable.isTreeExpandByRow(parentRow)) {
        ...
        columns.forEach(function (column, columnIndex) {
          ...
          if (exportLabelMethod) {
            ...
            switch (column.type) {
              ...
              case 'checkbox':
                ...
                _item._checkboxLabel = checkboxOpts.labelField ? _ctor.default.get(row, checkboxOpts.labelField) : '';
              ...
              case 'radio':
                ...
                _item._radioLabel = radioOpts.labelField ? _ctor.default.get(row, radioOpts.labelField) : '';
          ...
          _item[column.id] = _ctor.default.toString(cellValue);
        });
...
function getTxtCellKey(now) {
  return "#".concat(now, "@").concat(_ctor.default.uniqueId());
}

function replaceTxtCell(cell, vMaps) {
  return cell.replace(/#\d+@\d+/g, function (key) {
    return _ctor.default.hasOwnProp(vMaps, key) ? vMaps[key] : key;
  });
}
...
function handleFileImport($xetable, file, opts) {
  ...
  if (!_ctor.default.includes(_vXETable.default.importTypes, type)) {
    if (opts.message !== false) {
      _vXETable.default.modal.message({
        message: _conf.default.i18n('vxe.error.notType', [type]),
        status: 'error'
      });
    }
    ...
    fileInput.onchange = function (evnt) {
      ...
      if (!errType) {
        ...
        if (options.message !== false) {
          _vXETable.default.modal.message({
            message: _conf.default.i18n('vxe.error.notType', [errType]),
            status: 'error'
          });
        }
...
var _default = {
  methods: {
    ...
    _exportData: function _exportData(options) {
      ...
      if (customCols) {
        groups = _ctor.default.searchTree(_ctor.default.mapTree(customCols, function (item) {
          var targetColumn;
        ...
        groups = _ctor.default.searchTree(isGroup ? tableGroupColumn : tableFullColumn, function (column, index) {
      ...
      _ctor.default.eachTree(groups, function (column) {
        var isColGroup = column.children && column.children.length;
      ...
      if (!_ctor.default.includes(_vXETable.default.exportTypes, type)) {
        if (process.env.VUE_APP_VXE_TABLE_ENV === 'development') {
          _tools.UtilTools.error('vxe.error.notType', [type]);
        }
    ...
    _importData: function _importData(options) {
      ...
      var opts = Object.assign({
        types: _vXETable.default.importTypes // beforeImportMethod: null,
